...
library LibClock {
    function wrap(
        ...
        assembly {
            ...
            // bit  | 0 ... 63 | 64 ... 127
            /// @dev Combines timestamp and duration into a single uint256 value using bitwise operations.
            /// The duration is shifted left by 64 bits (0x40) and then OR'd with the timestamp.
            /// This packs both values into a single storage slot for efficient storage.
            /// The lower 64 bits contain the timestamp, and the upper 64 bits contain the duration.
            /// @dev Encodes a timestamp and duration into a single uint256 value.
            /// Uses bitwise operations to pack the data: duration is shifted left by 64 bits and combined with timestamp.
            /// shl (shift left) moves bits to the left, filling with zeros on the right.
            /// Format: [duration (upper 64 bits)][timestamp (lower 64 bits)]
